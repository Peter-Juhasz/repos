...
[TestClass]
public class StreamingBlobCollectionRepositoryTests(TestContext testContext)
{
	...
	private ICollectionRepository<Item> CreateRepository(IEqualityComparer<Item>? comparer = null) =>
		new StreamingBlobCollectionRepository<Item>(Blob, new JsonSerializerOptionsJsonCollectionSerializer<Item>(JsonSerializerOptions.Web), comparer);
	...
	{
		var repository = new StreamingBlobCollectionRepository<Item>(Blob, new JsonSerializerOptionsJsonCollectionSerializer<Item>(JsonSerializerOptions.Web));
	...
	[TestMethod]
	public void AsStreamingJsonCollectionRepository_CreatesStreamingRepository()
	{
		var repository = Blob.AsStreamingJsonCollectionRepository<Item>(JsonSerializerOptions.Web);

		Assert.IsInstanceOfType<StreamingBlobCollectionRepository<Item>>(repository);
	}
