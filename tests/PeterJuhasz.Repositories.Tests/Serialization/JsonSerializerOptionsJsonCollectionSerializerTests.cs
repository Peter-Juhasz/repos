...
[TestClass]
public class JsonSerializerOptionsJsonCollectionSerializerTests(TestContext testContext)
{
	...
	private static readonly ICollectionSerializer<JsonTestItem> Serializer = JsonSerializerOptionsJsonCollectionSerializer<JsonTestItem>.Web;
	...
	[TestMethod]
	public void Default_UsesDefaultOptions()
	{
		Assert.AreEqual("""[{"Name":"a","Count":1},{"Name":"b","Count":2}]""", Serialize(JsonSerializerOptionsJsonCollectionSerializer<JsonTestItem>.Default, Items));
	}

	[TestMethod]
	public void Strict_RejectsUnknownProperties()
	{
		var serializer = JsonSerializerOptionsJsonCollectionSerializer<JsonTestItem>.Strict;

		Assert.Throws<JsonException>(() => serializer.Deserialize("""[{"Name":"a","Count":1,"Extra":true}]"""u8, out _));
	}
