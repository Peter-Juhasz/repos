...
[TestClass]
public class JsonSerializerOptionsJsonSerializerTests(TestContext testContext)
{
	...
	private static readonly ISerializer<JsonTestItem> Serializer = new JsonSerializerOptionsJsonSerializer<JsonTestItem>(JsonSerializerOptions.Web);
	...
	[TestMethod]
	public void Serialize_UsesOptions()
	{
		var serializer = new JsonSerializerOptionsJsonSerializer<JsonTestItem>(JsonSerializerOptions.Default);

		Assert.AreEqual("""{"Name":"value","Count":42}""", Encoding.UTF8.GetString(serializer.SerializeToBufferWriter(Value)));
	}
